...
%sharat@mit.edu
%-------------------------------------------------------------------------
function model = train_dictionary_color(img_set,dsz,filtsz,nfilt,MAXITER)
	if(nargin<5)
		MAXITER = 10;
	end;
	%-----------------------------------
	...
	%initialize filters
	for i=1:nfilt
		filt{i}=0.1*randn(filtsz,filtsz,3);
	end;
	%-------------------------------
	...
	lrate  = 0.1;lstep = 0.05;		
 	for iter = 1:MAXITER
		...
		for p=1:length(dset)
			...
			for f=1:nfilt
			     den= conv2(ones(filtsz,1),ones(filtsz,1),sum(dset{p}.^2,3),'valid');
			     den= sqrt(den*sum(filt{f}(:).^2)+eps);
			     [pht,pwt,pd]=size(dpset{p});
			     num= zeros(pht-filtsz+1,pwt-filtsz+1);
			     for d=1:3
			       num = num+conv2(dset{p}(:,:,d),filt{f}(end:-1:1,end:-1:1,d),'valid');
			     end;
			     out= num./den;
			     mx = max(out(:));
			     [mi,mj]= find(out==mx,1);
