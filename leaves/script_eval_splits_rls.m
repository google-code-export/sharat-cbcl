...
%DIRS = {'CroppedAnimalFeaturesScales'};
SPLITS    = 1;
MAXFTR    = 1000;
sel       = [];
for dataset=1:length(DIRS)
	...
	for s=randperm(SPLITS)
		...
		split_file=fullfile(TARGET,sprintf('split_c2_%03d_results3.mat',s));
		split_lock = fullfile(TARGET,sprintf('evaluate_%03d.lock',s));
		model_file = fullfile(TARGET,sprintf('classif3_%03d.mat',s));
		if(~exist(trn_file) )
		...
		IDX=1:size(X,2);%IDX=randperm(size(X,2));IDX=IDX(1:6000);
