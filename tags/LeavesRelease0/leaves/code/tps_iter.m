...
%M>=N
%transforms
%based on Belongie's code
%sharat@mit.edu

function [sc_cost,aff_cost,match_cost]=tps_iter(X,Y)
display_flag=1;
...
r_outer=2;
n_iter=3;
r=0.99; % annealing rate
beta_init=1;  % initial regularization parameter (normalized)


if display_flag
   [x,y]=meshgrid(linspace(-0.5,0.5,18),linspace(-0.5,0.5,36));
   x=x(:);y=y(:);M=length(x);
end
...
%   cvec=hungarian_fast(costmat2);
   ...
   % Chui actually does mean of non-squared distance
    mse1=0;%mean(sqrt((y2(:,1)-Z(:,1)).^2+(y2(:,2)-Z(:,2)).^2));
