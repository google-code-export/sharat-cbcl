...
// $HeadURL: svn://iLab.usc.edu/trunk/saliency/src/NeovisionII/nv2_label_server.c $
// $Id: nv2_label_server.c 9352 2007-08-04 05:00:33Z rjpeters $
//
...
static void* run_patch_reader(void* vq)
{
        ...
        while (1)
        {
                ...
                const uint32_t npixbytes =
                        p.width * p.height * nv2_pixel_type_bytes_per_pixel((nv2_pixel_type)p.type);
                ...
                if (npixbytes > maxbytes)
                {
                        if (q->m_verbosity >= 1)
                                fprintf(stderr,
                                        ...
                                        (unsigned int) nv2_pixel_type_bytes_per_pixel((nv2_pixel_type)p.type),
