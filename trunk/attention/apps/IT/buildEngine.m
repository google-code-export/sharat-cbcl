...
%---------------------------------------------------------------------------------------
function engine=buildEngine(lSizes,xSigma,sSigma,pO,type)
  ...
  %build the network
   ...
   %connectivity	
			...
			for lp=1:prod(lSizes{ls})
				...
				for cs=1:NSCALES
					for cp=1:prod(lSizes{cs})
						...
						ds      =ls-cs;
						vals    =exp(-(ds.^2)/(2*sigmas^2));
						tbl(lstart+lp,1,cstart+cp)= max(EPS,valx)*max(EPS,vals);
						tbl(lstart+lp,2,cstart+cp)= EPS/NLOC;
					end;%end cp
					cstart = cstart+prod(lSizes{cs});
				end;%end cs
				tbl(lp+lstart,1,NLOC+1)= 0;
    ...
    switch(type)
        ...
        case 'bp'
            engine=pearl_inf_engine(bnet,'protocol','parallel','filename','/dev/null','storebel',1);
        case 'gibbs'
            engine=gibbs_sampling_inf_engine(bnet,'T',1000);
    end;           
