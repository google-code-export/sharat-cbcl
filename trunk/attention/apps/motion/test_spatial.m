...
%setup stimulus
%
dotleft=mkDots([NLOC*sz sz*NLOC depth],pi,0.5,0.1,1,-1,'exact');
dotright=mkDots([NLOC*sz sz*NLOC depth],0,0.5,0.1,1,-1,'exact');
dotup=mkDots([NLOC*sz sz*NLOC depth],pi/2,0.5,0.1,1,-1,'exact');
dot  =dotleft;
dot(2*sz+[1:sz],:,:)=dotup(2*sz+[1:sz],:,:);
dot(3*sz+1:end,:,:)=dotright(3*sz+1:end,:,:);

stim{1}=dot;
pL{1} = zeros(NLOC,NLOC);
pL{1}(1:2,:)=1;
flipBook(stim{1});
...
for i=1:length(stim)
    ...
    for t=1:size(res,3)
        ...
        for n=1:NFTR
            ...
            sevidence{C_start+n}=[map(:)>0.5;0.1];
            sevidence{F_start+n}=[0.5 0.5];
            sevidence{L}=pL{i}(:)/sum(pL{i}(:));
      end;
      ...
      subplot(5,1,4);imagesc(reshape(pL{i},[NLOC NLOC]),[0 1]);axis image off;
