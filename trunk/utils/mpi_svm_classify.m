...
%
%-----------------------------------------------------
function [y,yval] = mpi_svm_classify(X,model,comm,callback)
    %------------------------
    %mpi_stuff
    %------------------------
    source       =  0;
    ...
    [D,N]        =  size(X);
    batch_size   =  min(50,ceil(N/(mpi_size-1+eps)))
	...
	if(~isempty(tbl(i).idx))
	  ...
	  [lbl,val] = feval(callback,X(:,tbl(i).idx),model);
	else
	  fprintf('Empty matrix..doing nothing\n');
	  lbl = [];
	  val = [];
	end;
	MPI_Send(source,tbl(i).otag,comm,lbl,val);
    ...
    if(mpi_rank == source)
      y     = zeros(1,N);
      for i = 1:length(tbl)
	[lbl,val]          = MPI_Recv(tbl(i).dest,tbl(i).otag,comm);
	y(tbl(i).idx)      = lbl;
	yval(:,tbl(i).idx) = val;
