...
%sharat@mit.edu
%-------------------------------------------------------------------------
function model = train_dictionary_dist(img_set,dsz,filtsz,nfilt,MAXITER)
	if(nargin<5)
		MAXITER = 10;
	end;
	%-----------------------------------
	...
	dset = dset(idx(1:min(length(dset),50000)));		
	clear img_set;
	%-----------------------------------
	%initialize filters
	for i=1:nfilt
	...
	lrate  = 0.1;lstep = 0.05;		
 	for iter = 1:MAXITER
		...
		for p=1:length(dset)
			...
			for f=1:nfilt
			    dist    = conv2(ones(filtsz,1),ones(filtsz,1),dset{p}.^2,'valid');
				dist    = dist+sum(filt{f}(:).^2);
				dist    = dist-2*conv2(dset{p},filt{f}(end:-1:1,end:-1:1),'valid');
				out     = dist;
				mx      = min(out(:));
				[mi,mj] = find(out==mx,1);
				if(mx<mxval)
